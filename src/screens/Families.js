...
const Families = ({ photos = [], updatePhotos }) => {
        ...
        useEffect(() => {
          if (!photos || !photos.length) {
            FlickrAPI.getPhotos(
              ...
              [SIZE_LABELS.LARGE, SIZE_LABELS.SMALL]
            ).then(
        ...
        const transformForAllFamilies = (result) => {
          const transform = (sizes, index) => {
            ...
            const miniPicture = sizes.find(
              (s) => s.label === SIZE_LABELS.SMALL
            );
            const galleryPicture = sizes.find(
              (s) => s.label === SIZE_LABELS.LARGE
            );
            const gallery = {
        ...
        return (
          <Page alternativeMenu={alternativeMenu} onBackToHome={() => setSelectedFamily(null)}>
            <Wrapper>
